			...
			<section class="card" style="width: 85%;">
				<header class="card-header">
					<div class="card-actions">
						<?php echo form_open('instructor/training/edit_course/'.$course_id, array("id"=>"frm_preview")); ?>
								...
								<ul class="filtersLeft">
									<li>
										...
										<?php foreach($libraries as $chapter):?>
											<?php if($chapter->parent == 0):?>
												<label class="radioBox" style="margin-top:20px"><h5><?php echo $chapter->title?></h5>
													<input onclick="changeSelect(this,<?php echo $chapter->id?>, <?php echo $chapter->exam_id?>)" id="library<?php echo $chapter->id?>" type="radio" name="radio" value="<?php echo $chapter->file_path?>">
											...
											<?php foreach($libraries as $page):?>
												<?php if($page->parent != 0 && $page->parent == $chapter->id && $page->exam_id == 0 && $page->quiz_id == 0):?>
													<label class="radioBox" style="margin-left:20px"><?php echo $page->title?>
														<input onclick="changeSelect(this,<?php echo $page->id?>, <?php echo $chapter->exam_id?>)" id="library<?php echo $page->id?>" type="radio" name="radio" value="<?php echo $page->file_path?>">
														<span class="checkmark"></span>
													</label>
												<?php endif;?>
											<?php endforeach;?>
...
<form id="form_library_insert" action="<?=base_url()?>instructor/training/insert_library" method="post">
...
<script>
	...
	function changeSelect(el,id,exam_id){
        if (exam_id == "0"){
            ...
            exam_url = '<?php echo base_url()?>company/<?=$company_url?>/training/examInstruction/'+exam_id;
