		...
		<div class="col-lg-12">
			<section class="card">
				...
				<div class="card-body">
					...
					<form id="form_category" action="<?php echo base_url(); ?>admin/category/create" method="POST">
...
<script>
	...
	function deleteCategory(id, wcm_cat_id) 
	{	
		(new PNotify({
			...
			$.ajax({
                url: 'category/delete/'+id+'/'+wcm_cat_id,
                type: 'POST',
	...
	jQuery(document).ready(function() { 
		...
		$table.dataTable({
			...
			"ajax": {
                "type": "POST",
                "async": true,
				"url": "<?php echo base_url()?>admin/category/getlist",
				"data": '',		
				"dataSrc": "data",
				"dataType": "json",
				"cache":    false,
            ...
            "columnDefs": [ {
				...
				"createdCell": function (td, cellData, rowData, row, col) {
				    ...
				    if (rowData['name'] != "Online Courses" && rowData['name'] != "Live Classes" && rowData['name'] != "ILT")
					    $(td).html('<a href="javascript:changeCategory('+cellData+')"><i class="fas fa-pencil-alt"></i></a><span class="w-20"></span><a href="javascript:deleteCategory('+cellData+','+rowData['wcm_cat_id']+')" class="delete-row"><i class="far fa-trash-alt"></i></a>');
					else
					    $(td).html('');
