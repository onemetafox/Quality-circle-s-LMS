			...
			<section class="card">
				<header class="card-header">
					<div class="card-actions">
                        <a href="<?=base_url()?>admin/coursecreation/view_certificate_history" class="btn btn-default">Certificate History </a>
                        <a href="<?=base_url()?>admin/coursecreation/view_course_history" class="btn btn-default">Course History </a>
						<a href="javascript:check_add()" class="btn btn-success"><i class="fas fa-plus"></i> <?=$term["createcourse"]?> </a>
						...
						<div class="col-xs-12 col-sm-5 col-md-4 col-lg-3" style="">
							<div class="sidebar clearfix m-b-20">
								<div class="main-block">
									<ul>
										<li class="<?php echo $status==1 ? 'active' : '' ?>"><a href="<?php echo site_url('admin/coursecreation/getList?status=1')?>" class="scroll">Active</a></li>
										<li class="<?php echo $status==2 ? 'active' : '' ?>"><a href="<?php echo site_url('admin/coursecreation/getList?status=2')?>" class="scroll">In Active</a></li>
										<li class="<?php echo $status==0 ? 'active' : '' ?>"><a href="<?php echo site_url('admin/coursecreation/getList?status=0')?>" class="scroll">Draft</a></li>
									</ul>
									</form>
						...
						<div class="col-xs-12 col-sm-7 col-md-8 col-lg-9">
                        	<div class="row">                            
                               ...
                               <div class="col-sm-6">
                                    ...
                                    <a href="<?=base_url()?>admin/coursecreation/getList" class="btn btn-default">Reset</a>
                               </div>
                            </div>
							<div class="row">
								<div class="col-sm-4">
									...
									<div class="row">
										<div class="col-sm-12 col-md-12 col-lg-12 text-xs-center text-sm-left" style="padding: 20px;">
											...
											<div style="width: 90%;text-align: right;">
                                            	<?php if($item['active'] == 1 && $item['course_type'] == 2){?>
...
<script>
    ...
    $("#category_id").on('change',(function () {
        window.location = '<?=base_url()?>admin/coursecreation/getList?category=' + $("#category_id").val();
    }));

    $("#standard_id").on('change',(function () {
        window.location = '<?=base_url()?>admin/coursecreation/getList?standard=' + $("#standard_id").val();
    }));


	$("#course_type").on('change',(function () {
	    if($("#course_type").val() == "-1"){
            window.location = '<?=base_url()?>admin/coursecreation/getList';
        }
        else {
            window.location = '<?=base_url()?>admin/coursecreation/getList?course_type=' + $("#course_type").val();
        }
	}));
	
	$("#filter").on('click',(function () {
        window.location = '<?=base_url()?>admin/coursecreation/getList?course=' + $("#courses_filter").val();
    }));
    ...
    function check_add(){
	      var url = "<?= base_url()?>admin/coursecreation/check_add_view";
	      $.ajax({
	      url: url,
	      type: 'POST',
	      processData:false,
	      ...
	      success: function (data, status, xhr) {
	        if(data.success){
	              location.href="<?= base_url()?>admin/coursecreation/edit_course";
	        }else{
	          swal({
	            text: data.msg,
	            icon:"warning",
