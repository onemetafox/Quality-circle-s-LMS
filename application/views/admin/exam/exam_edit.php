	...
	<div class="row">
		<div class="col-lg-12">
			<section class="form-wizard" id="exam_wizard">
				<?php echo form_open('admin/exam/add_exam_auto', array("id"=>"frm_exam")); ?>
				<header class="card-header" style="background-color: #DAD9D9;">
					...
					<div class="card-actions">
						...
						<a class="btn btn-default" href="<?php echo base_url()?>admin/exam/showPreviewQuestion/<?=$exam['id']?>" style="margin-right: 10px;"><i class="fa fa-eye"></i> Preview</a>
								...
								<div id="quiz_type_div" class="card-body">
									<div class="row">
										<div class="col-md-6 mb-3">
											<button type="button" class="btn btn-default btn-block redirect" data-goto="multi-choice"><i class="fa fa-dot-circle-o"></i> Multiple choice</button>
										</div>
										<div class="col-md-6 mb-3">
											<button type="button" class="btn btn-default btn-block redirect" data-goto="checkbox"><i class="fa fa-check-square-o"></i> Checkbox</button>
										</div>
									</div>
									<div class="row">
										<div class="col-md-6 mb-3">
											<button type="button" class="btn btn-default btn-block redirect" data-goto="true-false">
												<i class="fa fa-check"></i>/<i class="fa fa-close"></i> True/False</button>
										</div>
										<div class="col-md-6 mb-3">
											<button type="button" class="btn btn-default btn-block redirect" data-goto="fill-blank"><i class="fa fa-minus"></i> Fill In the blank</button>
										</div>
									</div>
									<div class="row">
										<div class="col-md-6 mb-3">
											<button type="button" class="btn btn-default btn-block redirect" data-goto="matching"><i class="fa fa-server"></i> Matching</button>
										</div>
									</div>
								</div>
							</section>
						...
						<div class="form-group">
							<div class="col-sm-12">
								<div class="control-group" style="border-bottom: 1px solid #C2C2C2;" onclick = "exam_title_modal()">
									...
									<div id="exam_description_label" class="control-label col-lg-12" style="font-size: 15px;height: 100px;overflow-y: scroll;" for="textinput-0">
										...
										<li class="ui-state-default" data-id = "<?=$question['id']?>">
											...
											switch ($question['type']) {
												...
												case 'true-false':
													...
													break;
												case 'fill-blank':
													$checkData = array('blank'=>$question['content']['blank']);
													$this->load->view('admin/exam/subviews/fill_blank', $checkData);
													break;
												case 'essay':
...
<script>
	...
	function createExam(){
		...
		C.open("POST", base_url + 'admin/exam/save_exam_title');
		C.onload = function() {
			var E;
	...
	$(document).ready(function(){
		...
		$('.redirect').click(function() {
			...
			var controller = 'admin/exam/';
			switch(redirect) {
				case 'multi-choice':
					url = "multiChoice/"+exam_id;
					break;
				case 'checkbox':
					url = "checkbox/"+exam_id;
					break;
				case 'true-false':
					url = "trueFalse/"+exam_id;
					break;
				case 'fill-blank':
					url = "fillBlank/"+exam_id;
					break;
				case 'matching':
					url = "matching/"+exam_id;
					break;
				default:
					url = '';
			}
		...
		$('.clone-ques').click(function() {
			var ol = $('ol.quiz');
			var $this = $(this);
			var actionUrl = '<?php echo base_url()?>admin/exam/cloneQuestion';
			var id = $(this).data('id');
		...
		$('.remove-question').click(function() {
			var $this = $(this);
			var actionUrl = '<?php echo base_url()?>admin/exam/removeQuestion';
			var id = $(this).data('id');
			$.post( actionUrl, {id:id},function( data ) {
	...
	function updatePagePostion(id,startpos,newpos){
		...
		var url = "<?php echo base_url() ?>admin/exam/update_position";
		$.ajax({
			url: url,
			type: 'POST',
			data: formData,
