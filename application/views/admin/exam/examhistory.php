	...
	jQuery(document).ready(function(){	
		...
		var tbdata = $history_table.dataTable({
            ...
            },
            {
                "targets": [8],
                "createdCell": function (td, cellData, rowData, row, col) {
                    $(td).html('<a class="btn btn-default btn-sm" href="<?php echo base_url()?>admin/examhistory/preview_history/'+cellData+'">Preview</a><span class="w-20"></span>');
                    $(td).append('<a class="btn btn-default btn-sm" href="feedback/'+cellData+'">Feedback</a><span class="w-20"></span>');
                 
                    if (rowData["type"] == "Manual"&&rowData["exam_status"] == "Not Determine"){
                        $(td).append('<a class="btn btn-default btn-sm" href="<?php echo base_url()?>admin/examhistory/set_mark/'+cellData+'">Set Mark</a><span class="w-20"></span>');
                    }
                }
            }],
	        "columns": [
