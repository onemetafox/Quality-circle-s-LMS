...
<style>
.endline{
    width:100%;border: 2px solid red;position: absolute;right: 0;top:1360px;z-index: 8
}
.mce-content-body{
...
<section role="main" class="content-body">
    <header class="page-header">
        ...
        <div class="card-actions" style="margin-top: 7px !important; top:initial">
            ...
            <a class="btn btn-primary modal-with-form" href="<?=base_url()?>admin/library" ><?=$term[librarylist]?>
            </a>
    ...
    <div class="row editor-row" style="position: relative;">
...
<script>
    ...
    function  export_btn() {
/*        $('td').css('break-before','avoid');
        $('table').css('break-before','avoid');
        $('tbody').css('break-before','avoid');
        $('tr').css('break-before','avoid');*/
        var arr=[];
        var count=0;
        var _html = '';
...
//        }
    ...
    _emailBuilder.setAfterLoad(function(e) {
        ...
        $('.content-main .sortable-row-content').html('<?=str_replace("'","",$content)?>');
        <?php endif;?>
        $(".left-menu-container").css("z-index",9);
        var over_height = 0;
        var limit_height = 1250;
        var endline_count = 0;
        $("#editorContent").on('mouseover',function(){
            over_height = $("#editorContent .content-main").height();
            if(over_height < limit_height){
                $(".endline").remove();
            }else if(over_height >= limit_height){
                $(".endline").remove();
                endline_count = Math.floor(over_height/limit_height);
                if(endline_count > 0){
                    for(i=0; i<endline_count; i++){
                        $(".editor-row").append('<div class="endline" style="top:'+limit_height*(i+1)+'px"></div>');
                    }
                }
            }
        });
