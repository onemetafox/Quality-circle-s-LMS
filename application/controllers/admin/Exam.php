...
class Exam extends BaseController {
    ...
    public function __construct(){
        ...
        $side_params = array('selected_menu_id' => '44-1');
    ...
    public function index(){
        if($this->isMasterAdmin()){
            $this->loadViews("admin/exam/exam_list", $this->global, NULL, NULL);
        }else{
    ...
    public function viewQuizList(){
        $side_params = array('selected_menu_id' => '14-1');
        $this->global['sidebar'] = $this->sidebar->generate($side_params, $this->role);
        if($this->isMasterAdmin()){            
            $this->loadViews("admin/exam/quiz_list", $this->global, NULL, NULL);
        }else{
            $this->loadViews("access", $this->global, NULL, NULL);
        }
    }
    ...
    public function viewQuizGroupList(){
        $side_params = array('selected_menu_id' => '14-2');
        $this->global['sidebar'] = $this->sidebar->generate($side_params, $this->role);
        if($this->isMasterAdmin()){           
            $this->loadViews("admin/exam/quiz_group_list", $this->global, NULL, NULL);
        }else{
            $this->loadViews("access", $this->global, NULL, NULL);
        }
    }
            ...
            if($type == null){
                ...
                switch ($this->global['qData']['type']){
                    case 'multi-choice':
                        ...
                        $this->loadViews("admin/exam/multi_choice", $this->global, NULL, NULL);
                    break;
                    case 'checkbox':
                        $this->global['title'] = 'Checkbox';
                        $this->loadViews("admin/exam/checkbox", $this->global, NULL, NULL);
                    break;
                    case 'true-false':
                        $this->global['title'] = 'True / False';
                        $this->loadViews("admin/exam/true_false", $this->global, NULL, NULL);
                    break;
                    case 'fill-blank':
                        $this->global['title'] = 'Fill in the Blank';
                        $this->loadViews("admin/exam/fill_blank", $this->global, NULL, NULL);
                    break;
                    case 'essay':
                        $this->global['title'] = 'Essay';
                        $this->loadViews("admin/exam/essay", $this->global, NULL, NULL);
                    break;
                    case 'matching':
                        $this->global['title'] = 'Matching';
                        $this->loadViews("admin/exam/matching", $this->global, NULL, NULL);
                    break;
                    default:
                }
        }else{
    ...
    public function addMultiRow(){
        ...
        $this->load->view('admin/exam/multiple-choice-row', $data);
    }

    public function addCheckboxRow(){
        $data['count'] = $this->input->post('count');
        $this->load->view('admin/exam/checkbox-row', $data);
    }

    public function addBlankRow(){
        $data['count'] = $this->input->post('count');
        $this->load->view('admin/exam/blank-row', $data);
    }

    public function addMatchingRow(){
        $data['count'] = $this->input->post('count');
        $this->load->view('admin/exam/matching-row', $data);
    }
    public function saveQuestion(){
        ...
        $quesdata['pass_mark'] = $this->input->post('pass_mark');
        $quesdata['max_mark'] = $this->input->post('max_mark');
        switch ($type){
            case 'multi-choice':
        ...
        if($update != ''){
            ...
            $this->Exam_model->updateQuiz($quesdata, $exam_quiz_id);
        }else{
            $quesdata['position'] = $this->input->post('position');
            $quiz_id = $this->Exam_model->insertQuiz($quesdata);
        }
        if($this->input->post('is_only_quiz') == 1){
            redirect('admin/exam/viewQuizList');
        }else{
            redirect('admin/exam/create?row_id=' . $exam_id);
        }
    }
    ...
    public function showPreviewQuestion($exam_id = 0){
        if($this->isMasterAdmin()){
            ...
            if($this->input->post('id') != ''){
                $this->load->view('admin/exam/showPreviewQuestion', $this->global);
            }else{
                $this->loadViews("admin/exam/showPreviewQuestion", $this->global, NULL, NULL);
            }
        }else{
            $this->loadViews("access", $this->global, NULL , NULL);  
        }
    ...
    function reportCard($exam_id = 0){
        if($this->isMasterAdmin()){
            ...
            $this->loadViews("admin/exam/reportCard", $this->global, NULL, NULL);
        }else{
            $this->loadViews("access", $this->global, NULL , NULL);  
        }
    }
	
	public function reportCardView($exam_id = 0){
		if($this->isMasterAdmin()){
			$this->global['sidebar'] = '';
            $user_id = $this->session->get_userdata() ['user_id'];
            $this->global['user_name'] = $this->session->get_userdata() ['user_name'];
            ...
            $this->global['certificate'] = str_replace("{EXAM_TITLE}", $this->Exam_model->getRow($exam_id) [0]['title'], $this->global['certificate']);
            $this->global['certificate'] = str_replace("{EXAM_SCORE}", $this->global['score'], $this->global['certificate']);
            $this->loadViews("admin/exam/reportCardView", $this->global, NULL, NULL);
        }else{
            $this->loadViews("access", $this->global, NULL , NULL);  
        }
	}
	
	public function examInstructionView($exam_id=0,$course_id = 0){
        ...
        $this->load->view('admin/exam/exam-instruction', $params);
    }
