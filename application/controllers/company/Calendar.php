...
class Calendar extends BaseController
{
    ...
    {
        // die('Training Calendar Page Comes Here');
    ...
    public function createCourse()
    {
        ...
        if ($rslt['possible'] == 1){
            ...
            $course_data['attend_img'] = str_replace("./", "", $upload_path.'default.png');
    ...
    public function showTraining($d = 0)
    {
        ...
        foreach ($res_id_list as $key => $row) {
            foreach($result_list as $k => $r){
                if($r['training_course_id'] == $row['id']){
                    if(!array_key_exists($r['month'],$time_list[$row['id']]))
                        $time_list[$row['id']][$r['month']] = array();
        ...
        $training_data['company'] = $this->company;
        ...
        $this->loadViews_front("company_page/training-calendar", $training_data);
