...
class Demand extends BaseController{
    ...
    public function viewCreate(){
        if($this->isInstructor()){
            ...
            //            $page_data['cert_temp_list'] = $this->Settings_model->getCertificate();
            if ($row_id != 0){
                ...
                $exam_row['created_at'] = '';
                $exam_row['updated_at'] = '';
                //$cert_temp_data = $this->Settings_model->getCertificate();
                $page_data['exam'] = $exam_row;
            }
            //            $page_data['training_topic'] = $this->Topic_model->getAllList();
            if ($exam_type == "Auto"){
    ...
    public function getList(){
        if($this->isInstructor()){
            ...
            $config['base_url'] = site_url('instructor/demand/getList?status=' . $status . '&sSearch=' . $search);
            ...
            $this->loadViews("instructor/demand/course_list", $this->global, NULL, NULL);
        }else{
            $this->loadViews("access", $this->global, NULL, NULL);
        }
    }
    ...
    public function preview_course($row_id = 0){
        ...
        $page_path = "instructor/demand/preview_course";
    ...
    public function view_course($id = NULL){
        if($this->isInstructor()){
            $page_path = "instructor/demand/view_course";
    ...
    public function view_row_assess($course_id = 0, $user_id = 0){
        if($this->isInstructor()){
            ...
            $page_data['course_pay_user'] = $this->Course_model->getAssessUser($course, $user_id);
            ...
            $this->loadViews("instructor/demand/view_assess", $this->global, $page_data, NULL);
        }else{
            $this->loadViews("access", $this->global, NULL, NULL);
        }
    }
    ...
    public function view_exam_history($course_id, $user_id){
        if($this->isInstructor()){
            ...
            $this->loadViews("instructor/demand/preview", $this->global, NULL, NULL);
        }else{
            $this->loadViews("access", $this->global, NULL, NULL);
        }
    }
    ...
    public function detail_course($row_id = 0){
        if($this->isInstructor()){
            $page_path = "instructor/demand/detail_course";
            $course = $this->Course_model->select($row_id);
            $this->global['libraries'] = $this->Course_model->getLibrary($row_id);
            $this->global['course_name'] = $course->title;
            $this->global['course_id'] = $row_id;
    ...
    public function save_course_profile(){
        ...
        $course_data['location'] = $this->input->post('location');
        ...
        $course_data['number'] = $this->input->post('number');
        ...
        redirect('instructor/demand/getList');
    }
    ...
    public function edit_course_tab($row_id = 0, $tab_id = 1){
        ...
        $lang_ar = $this->Translate_model->getLanguageList(array('active_flag' => 1, 'add_flag' => 1));
        $page_data['lang_ar'] = $lang_ar['data'];
        $page_path = "instructor/demand/course_edit";
        if ($row_id != 0){
            $course = $this->Course_model->select($row_id);
            $this->global['libraries'] = $this->Course_model->getLibrary($row_id);
            $this->global['course_name'] = $course->title;
    ...
    public function edit_course($row_id = 0){
        if($this->isInstructor()){
            ...
            $this->global['exam_data'] = $this->Exam_model->getExamListByCompanyID($this->session->get_userdata() ['company_id']);
            $userdata = $this->Course_model->getUsers();
            foreach($userdata as $res){
                $sender_id = $this->session->userdata('userId');
                $notif_data['notification_type'] = "normal";
                $notif_data['from_user_id'] = $sender_id;
                $notif_data['to_user_id'] = $res->id;
                $notif_data['notification_title'] = "Receive Message";
                $notif_data['notification_message'] = "New Course created";
                $notif_data['created_at'] = date("y-m-d H:i:s");
                $this->db->insert('notification', $notif_data);
            }
            $this->loadViews($page_path, $this->global, NULL, NULL);
        }else{
            $this->loadViews("access", $this->global, NULL, NULL);
    ...
    public function create_temp_page(){
        ...
        $data = array('course_id' => $this->input->post('course_id'), 'parent' => $cid, 'page_type' => $this->input->post('page_type'), 'title' => 'Page ' . $page, 'description' => 'Add Text Here', 'position' => $ids,);
        $result = $this->Course_model->data_insert('chapter', $data);
        if ($result){
    ...
    public function create_temp_page_id(){
        ...
        $ids = ($check[0]->position) + 1;
        $page = $ids;
        $data = array('course_id' => $this->input->post('course_id'), 'parent' => $this->input->post('parent'), 'page_type' => $this->input->post('page_type'), 'title' => 'Page ' . $page, 'description' => 'Add Text Here', 'position' => $ids,);
        $result = $this->Course_model->data_insert('chapter', $data);
        if ($result){
    ...
    public function view_course_history(){
        if($this->isInstructor()){
            $this->loadViews("instructor/demand/course_history", $this->global, null, NULL);
        }else{
            $this->loadViews("access", $this->global, NULL, NULL);
        }
    }
    ...
    public function view_certificate_history(){
        if($this->isInstructor()){
            $this->loadViews("instructor/demand/certificate_history", $this->global, null, NULL);
        }else{
            $this->loadViews("access", $this->global, NULL, NULL);
        }
    }
