...
class Eveningwrk extends BaseController {
    ...
    public function viewList(){
        ...
        if($this->isLearner()){
            ...
            $this->loadViews("learner/eveningwrk/index", $this->global, null, null);
        }else{
            $this->global['sidebar'] = $this->sidebar->generate($side_params, $this->role);
    ...
    public function viewCreate(){
        ...
        if($this->isLearner()){            
			...
			$page_data["allstudents"] = $this->EveningWrk_model->getListAllByStudent();
            $this->loadViews("learner/eveningwrk/excercise_edit", $this->global, $page_data, null);
        }else{
            $this->loadViews("access", $this->global, null, null);
        }
    }
    ...
    public function getList(){
        ...
        $out_data = [];
		$table_data['data'] = $this->EveningWrk_model->getListAllByStudent();
	...
	public function saveExcercise(){
		...
		$video_data['student_id'] = $this->session->userdata('user_id');
        if(intval($video_data["id"]) == 0){
            unset($video_data["id"]);
        ...
        redirect('learner/eveningwrkexcercise');
    }
