...
(function (factory) {
  ...
  var Editor = function () {
     ...
     * @param {String} sUrl
     */
    this.insertNode = function ($editable, el, inline) {
	  $node = $(el).appendTo(document.body);
      $editable.focus();
      rng = range.create();
	  rng.deleteContents();
	  rng.insertNode($node[0],inline);
	  if($node.parents(".note-editable").length==0)
		  $node.appendTo($editable);
      afterCommand($editable);
    };

	this.getRange = function ($editable) {
      $editable.focus();
      var rng = range.create();
  ...
  var EventHandler = function () {
    ...
    var hToolbarAndPopoverClick = function (event) {
      ...
      if ($btn.length) {
        ...
        if (editor[eventName]) { // on command
        	...
        	opts = layoutInfo.editor().data('options');
        	if(opts.commands[eventName])
        		opts.commands[eventName].call(this, layoutInfo);
    ...
    this.attach = function (layoutInfo, options) {
      ...
      layoutInfo.editor.data('editor',editor);
  ...
  var Renderer = function () {
    ...
    var tplButtonCustom = function (options) {
        ...
        return '<button type="button"' +
                   ...
                   (hide ? ' data-hide=\'' + hide + '\'' : '') +
                   ' tabindex="-1">' +
                   	(icon ? '<i class="fa fa-' + icon + ' glyphicon glyphicon-' + icon + '"></i>' : '') +
                   	(label ? '\n' + label : '') +
                   	(dropdown ? ' <span class="caret"></span>' : '') +
               '</button>' + (dropdown || '');
      };
    ...
    this.createLayoutByFrame = function ($holder, options) {
      ...
      for (var idx = 0, len = options.toolbar.length; idx < len; idx ++) {
        if(options.toolbar[idx]=="->") {
			pulled = true;
		} else {
			var groupName = options.toolbar[idx][0];
			var groupButtons = options.toolbar[idx][1];

			toolbarHTML += '<div class="note-' + groupName + (pulled?' pull-right':'') + ' btn-group">';
			for (var i = 0, btnLength = groupButtons.length; i < btnLength; i++) {
			  if(groupName=="custom" && $.isPlainObject(groupButtons[i])) {
				  toolbarHTML += tplButtonCustom(groupButtons[i],options);
			  } else if ($.isFunction(tplButtonInfo[groupButtons[i]]))
